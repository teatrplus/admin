...
function slugify(value) {
  return Array.from(value.toLowerCase()).map((letter) => letters[letter] ?? letter).join('')
...
function assign(app, record) {
  ...
  const available = (candidate) => app.findRecordsByFilter(collection,
    'slug = {:slug} && id != {:id}', '', 1, 0, { slug: candidate, id: record.id }).length === 0
  if (!available(slug)) {
