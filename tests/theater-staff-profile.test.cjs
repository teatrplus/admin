...
test('staff profile migrations seed one director and restrict overrides to local paths', (t) => {
  ...
  fs.writeFileSync(path.join(migrations, '1788859990_fixture.js'), `migrate((app) => {
  ...
  for (const file of ['1788860000_staff_profile_path.js', '1788860010_seed_mikhail_doloko.js', '1788860040_artistic_director_department.js']) {
  ...
  fs.copyFileSync(path.join(root, 'pb_migrations/1788860010_seed_mikhail_doloko.js'), path.join(migrations, '1788860020_repeat_seed.js'))
  fs.writeFileSync(path.join(migrations, '1788860050_assertions.js'), `migrate((app) => {
  ...
  const result = spawnSync(process.env.POCKETBASE_BINARY || path.join(root, 'pocketbase'), [
    'migrate', 'up', `--dir=${path.join(temporary, 'data')}`, `--migrationsDir=${migrations}`, `--hooksDir=${path.join(temporary, 'hooks')}`,
  ], { encoding: 'utf8' })
