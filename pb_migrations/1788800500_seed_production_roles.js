...
migrate(
  (app) => {
    ...
    for (const person of people) {
      ...
      const existing = app.findRecordsByFilter('t_role', 'type = "production" && name_en = {:name}', '', 2, 0, {
        name: person.name_en,
      })
      ...
      const assigned = app.findRecordsByFilter('t_staff_role', 'staff = {:staff} && role = {:role}', '', 1, 0, {
        staff: staff.id,
        role: role.id,
      })
      if (!assigned.length) {
