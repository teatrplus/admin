...
migrate(
  (app) => {
    ...
    for (const record of app.findAllRecords('t_staff_role')) {
      ...
      if (!staff || !role || seen.has(pair))
        throw new Error(`Incomplete or duplicate staff-role assignment: ${record.id}`)
    ...
    assignments.fields.getByName('staff').help =
      'Person receiving this credit. Reuse the same person-role pair across plays.'
    assignments.fields.getByName('role').help =
      'Character or production responsibility. This assignment only appears on plays that select it in their roles field.'
    ...
    for (const [name, field, help] of [
      [
        't_staff',
        'role',
        'Department: actor, production or administration. This does not automatically credit the person on any play.',
      ],
      [
        't_role',
        'type',
        'actor = character; production = production responsibility; administration = administrative responsibility.',
      ],
      [
        't_play',
        'roles',
        'Confirmed participants: select person-role assignments from t_staff_role. Includes cast and production team; reuse assignments on other plays when appropriate.',
      ],
    ]) {
