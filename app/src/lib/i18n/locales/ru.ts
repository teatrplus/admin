import type { TranslationSchema } from '../schema'

export const ru: TranslationSchema = {
  publication: {
    publish: 'Опубликовать',
    publishing: 'Публикация…',
    pending: 'Неопубликованные изменения',
    landing: 'Лендинг',
    theater: 'Театр',
    started: 'Публикация началась. Изменения появятся на сайте примерно через 2 минуты.',
    failed: 'Не удалось начать публикацию. Изменения сохранены. Попробуйте ещё раз.',
    retryStatus: 'Проверить',
  },
  workspace: {
    label: 'Управление контентом',
    overview: 'Обзор',
    homeDescription: 'Истории, спектакли и детали, из которых складывается главная страница.',
    masksDescription: 'Коллекция масок и всё, что нужно знать посетителям музея.',
    inquiriesDescription: 'Сообщения посетителей — в одном месте.',
    staffDescription: 'Ваша команда и её доступ к рабочему пространству.',
    accountDescription: 'Личные данные, безопасность и доступ к рабочему пространству.',
    landingDescription: 'Страница площадки: от первого впечатления до важных деталей.',
    requestsDescription: 'Каждая заявка — от первого обращения до события.',
    socialDescription: 'Свежие публикации из Instagram на вашем сайте.',
    loginTitle: 'С возвращением.',
    loginBody: 'Войдите в рабочее пространство Театр+.',
    loginCaption: 'За каждым большим спектаклем.',
    loginStory: 'Одно место для контента, команды и деталей, которые объединяют всё.',
    loginFooter: 'Театр+ · Администрирование',
    profile: 'Личные данные',
    security: 'Пароль и безопасность',
    access: 'Доступ к пространству',
    securityHint: 'Оставьте поля пустыми, чтобы сохранить текущий пароль.',
    shortcuts: 'Ваше рабочее пространство',
    open: 'Открыть',
    museumContent: 'Содержимое музея',
  },
  inquiries: {
    title: 'Обращения',
    name: 'Имя',
    email: 'Email',
    phone: 'Телефон',
    message: 'Сообщение',
    created: 'Получено',
    status: 'Статус',
    todo: 'К выполнению',
    done: 'Выполнено',
    all: 'Все',
    empty: 'Обращений пока нет.',
    error: 'Не удалось загрузить обращения.',
    saveError: 'Не удалось изменить статус. Попробуйте ещё раз.',
    refresh: 'Обновить',
    previous: 'Назад',
    next: 'Далее',
    complete: 'Отметить выполненным',
    reopen: 'Вернуть к выполнению',
    loading: 'Загрузка…',
  },
  common: {
    brand: {
      beforePlus: 'Театр',
      afterPlus: ' Админ',
    },
    save: 'Сохранить',
    cancel: 'Отмена',
    add: 'Добавить',
    remove: 'Удалить',
    loading: 'Загрузка…',
    logout: 'Выйти',
    forbiddenTitle: 'Нет доступа',
    forbiddenBody: 'У вашей учётной записи нет прав для этой страницы.',
    backHome: 'На главную',
    language: 'Язык',
    toggleTheme: 'Переключить тему',
    notifications: 'Уведомления',
    saved: 'Сохранено',
    error: 'Ошибка',
  },
  home: {
    greeting: 'Привет, {name}',
    body: 'Выбери раздел в боковой панели, чтобы начать работу.',
  },
  scopes: {
    space: 'Пространство',
    theater: 'Театр',
  },
  auth: {
    email: 'Email',
    password: 'Пароль',
    submit: 'Войти',
    invalid: 'Неверный email или пароль',
  },
  nav: {
    general: 'Общее',
    content: 'Страницы сайта',
    homepage: 'Главная страница',
    masks: 'Маски и музей',
    inquiries: 'Обращения',
    sections: {
      space: 'Пространство',
      theater: 'Театр',
      global: 'Общее',
    },
    landing: 'Лендинг',
    requests: 'Заявки',
    staff: 'Сотрудники',
    account: 'Настройки аккаунта',
    social: 'Соцсети',
    collapse: 'Свернуть',
    expand: 'Развернуть',
  },
  landing: {
    title: 'Редактор лендинга',
    general: 'Основное',
    venue: 'Площадкa',
    advantages: 'Преимущества',
    process: 'Процесс',
    gallery: 'Галерея',
    partners: 'Партнёры',
    contacts: 'Телефоны в подвале',
    headerPhoneManager: 'Телефон в шапке',
    telegramManager: 'Менеджер Telegram',
    presentationUrl: 'Ссылка на презентацию',
    head: 'Заголовок',
    body: 'Текст',
    caption: 'Подпись',
    contentLocale: 'Язык контента',
    contentLocales: {
      ru: 'RU',
      en: 'EN',
      uz: 'UZ',
    },
    file: 'Файл',
    youtubeUrl: 'Ссылка YouTube',
    youtubeUrlHint: 'Вместо изображения можно указать ссылку на YouTube.',
    youtubeVideo: 'Видео YouTube',
    addYoutubeRow: 'Добавить ссылку на видео',
    dropHint: 'Перетащите изображения сюда или нажмите, чтобы выбрать',
    replaceImage: 'Заменить изображение',
    contactManagers: 'Номера сотрудников в подвале сайта',
    none: 'Не выбран',
    addRow: 'Добавить',
    removeRow: 'Удалить',
    deleteImage: 'Удалить изображение',
    savedToast: 'Сохранено. Нажмите «Опубликовать», чтобы обновить сайт.',
    validationHeaderPhone: 'Выберите контакт для телефона в шапке.',
    validationMinItems: 'Добавьте хотя бы один элемент в раздел «{section}».',
    validationItemLocales: 'Заполните все поля RU / EN / UZ в разделе «{section}».',
    validationGalleryMedia: 'У каждого элемента галереи должно быть изображение или ссылка YouTube.',
    validationGalleryBoth: 'Используйте либо изображение, либо ссылку YouTube, но не оба варианта.',
    validationYoutubeUrl: 'Введите корректную ссылку YouTube.',
    validationFailed: 'Исправьте выделенные поля перед сохранением.',
    noChanges: 'Нет изменений для сохранения.',
    missingPhoneToast: 'У этой учётной записи не указан номер телефона.',
    missingTelegramToast: 'У этой учётной записи не указан Telegram.',
  },
  requests: {
    title: 'Заявки',
    clientName: 'Клиент',
    clientPhone: 'Телефон',
    dateRequested: 'Дата',
    manager: 'Менеджер',
    stage: 'Статус',
    unassigned: 'Не назначен',
    archive: 'В архив',
    unarchive: 'В работу',
    archived: 'Заявка архивирована',
    unarchived: 'Заявка возвращена из архива',
    archivedSection: 'Архив',
    actions: 'Действия',
    archiveBlocked: 'Архивировать можно только отклонённые, завершённые или отменённые заявки.',
    viewBoard: 'Доска',
    viewTable: 'Таблица',
    empty: 'Нет заявок',
    emptyArchived: 'Нет архивных заявок',
    pageOf: 'Страница {page} из {total}',
    prevPage: 'Назад',
    nextPage: 'Вперёд',
    dateUpdated: 'Дата обновлена',
    stages: {
      inquiry: 'Запрос',
      confirmed: 'Подтверждено',
      rejected: 'Отклонено',
      preparation: 'Подготовка',
      completed: 'Завершено',
      cancelled: 'Отменено',
    },
  },
  account: {
    title: 'Мой Профиль',
    profile: 'Профиль',
    updated: 'Аккаунт обновлён',
    validationFailed: 'Исправьте выделенные поля перед сохранением.',
  },
  staff: {
    title: 'Сотрудники',
    create: 'Создать пользователя',
    edit: 'Редактировать',
    list: 'Список',
    email: 'Email',
    password: 'Пароль',
    passwordConfirm: 'Подтверждение пароля',
    passwordOptional: 'Оставьте пустым, чтобы не менять пароль',
    name: 'Имя',
    phoneNumber: 'Телефон',
    telegramUsername: 'Telegram',
    role: 'Роль',
    scope: 'Проект',
    roles: {
      admin: 'Администратор',
      moderator: 'Модератор',
      manager: 'Менеджер',
      viewer: 'Наблюдатель',
    },
    scopes: {
      theater: 'Театр',
      space: 'Пространство',
    },
    created: 'Пользователь создан',
    updated: 'Пользователь обновлён',
    deleted: 'Пользователь удалён',
    deleteConfirm: 'Удалить {name}? Это действие нельзя отменить.',
    editUser: 'Редактировать',
    deleteUser: 'Удалить',
    validationFailed: 'Исправьте выделенные поля перед сохранением.',
  },
  theater_social_panel: {
    loadingPosts: 'Загрузка постов Instagram…',
    postsError: 'Не удалось загрузить посты Instagram.',
    retry: 'Повторить',
    emptyPosts: 'Постов пока нет. Обновите посты Instagram, чтобы загрузить их.',
    postsLabel: 'Последние посты Instagram',
    postAlt: 'Пост Instagram',

    title: 'Соцсети',
    body: 'Обновите девять последних постов Instagram. Это может занять несколько минут; страница обновится автоматически, когда они будут готовы.',
    refresh: 'Обновить посты Instagram',
    started: 'Обновление запущено. Можно уйти со страницы — задача продолжит работать на сервере.',
    running: 'Обновление постов… запущено {time}',
    success: 'Обновлено постов Instagram: {count}.',
    successAt: 'Последнее обновление: {time} ({count} постов)',
    failed: 'Не удалось обновить: {error}',
    inProgress: 'Обновление уже выполняется.',
  },
  validation: {
    required: 'Обязательное поле',
    email: 'Некорректный email',
    minLength: 'Слишком короткое значение',
    passwordMatch: 'Пароли не совпадают',
    url: 'Некорректная ссылка',
  },
}
